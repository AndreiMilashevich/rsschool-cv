...
function timeInsert() {
  ...
  if (lang === "en") {
    ...
    dayOfWeek.forEach((item, i) => {
      item.textContent = dayOfWeekEn[(i + 1 + day) % daysInWeek];
    });
  } else {
    ...
    dayOfWeek.forEach((item, i) => {
      item.textContent = dayOfWeekRu[(i + 1 + day) % daysInWeek];
    });
  }
...
function setContentRu() {
  ...
  lonDOM.textContent = `долгота: ${Math.floor(longitude)}° ${Math.round(
    (longitude % 1) * minutesInOnedeg)}'`;
  latDOM.textContent = `широта: ${Math.floor(latitude)}° ${Math.round(
    (latitude % 1) * minutesInOnedeg)}'`;
  getCountryByCode();
...
async function setContentEn() {
  ...
  cityDOM.textContent = `${city}, ${countryName}`;
  feelsDOM.textContent = `Feels like: ${convertTemp(feels)}°`;
...
function getMap() {
  ...
  mapboxgl.accessToken =
    "pk.eyJ1Ijoiam9objQ4NDIiLCJhIjoiY2tqNGUwd3dyMGp5MjJ3bnZkaGg5NnE2dyJ9.U0xQhDaQracUXFn6vxkwUw";
...
async function getCountryByCode() {
  if (lang === "ru") {
    ...
    try {
      ...
      console.log(
        "Oops, something went wrong!!! Probably, such  country code does not exist!"
      );
    }
  }
}
...
async function getForecast() {
  try {
    ...
    console.log(
      "Oops, something went wrong!!! Probably, can not find weather  exist!"
    );
  }
}
...
navigator.geolocation.getCurrentPosition(
  getCoordsFromNavigator,
  getCoordsFromNavigatorError,
  mapOptions
);
setBackground();
getLang();
